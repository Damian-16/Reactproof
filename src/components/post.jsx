...
const Post = (item) => {
  ...
  const handleAddFavButton = (params) => {
    if (!item.fav) {
      ...
      const existingIndex = item.fav.findIndex(
        (favItem) => favItem.objectID === params.objectID
      );
      if (existingIndex === -1) {
        item.setFav([...item.fav, params]);
      } else {
        const updatedFav = item.fav.filter(
          (favItem, index) => index !== existingIndex
        );
  ...
  return (
    ...
    >
      <Link style={{width: '70%', }} href={story_url} textColor="none" underline="none">
